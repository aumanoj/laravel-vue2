...
class ZillaLikes {
	...
	function like_this($post_id, $zero_postfix = false, $one_postfix = false, $more_postfix = false, $action = 'get')
	{
		...
		switch($action) {
			...
			case 'get':
				...
				if($likes == 0)
					$css_class = "zilla-likes-count zero-class";
				else
					$css_class = "zilla-likes-count";

				return '<span class="' . $css_class . '">'. $likes .'</span> <span class="zilla-likes-postfix">'. $postfix .'</span>';
				break;
	...
	function do_likes()
	{
		...
		return '<a href="#" class="'. $class .'" id="zilla-likes-'. $post->ID .'" title="'. $title .'"><i class="os-icon-heart"></i> <span class="zilla-like-count">'. $output .'</span></a>';
	}
